...
def shift_rows(state):
    return [
        ...
        state[4],  state[9],  state[14], state[3],   # Row 1 (left shift by 1)
        state[8],  state[13], state[2],  state[7],   # Row 2 (left shift by 2)
        state[12], state[1],  state[6],  state[11]   # Row 3 (left shift by 3)
    ]
...
def mix_columns(state):
    ...
    for c in range(4):  # column-wise
        for r in range(4):  # each row element
            val = 0
            for k in range(4):
                val ^= mul(state[c*4 + k], mix_matrix[r][k])
            new_state[c*4 + r] = val
    return new_state
...
def key_expansion(key):
    ...
    # Start with the original 4 words (16 bytes)
    w = [key[i:i+4] for i in range(0, 16, 4)]

    for i in range(4, 44):  # need 44 words for AES-128
        temp = w[i - 1]
        if i % 4 == 0:
...
def aes_encrypt(plaintext_block, round_keys):
...
def inv_mix_columns(state):
    ...
    for c in range(4):  # column-wise
        for r in range(4):  # each row element
            val = 0
            for k in range(4):
                val ^= mul(state[c*4 + k], inv_matrix[r][k])
            new_state[c*4 + r] = val
    return new_state

def aes_decrypt(ciphertext_block, round_keys):
    state = add_round_key(ciphertext_block, round_keys[10])
...
def main():
    ...
    byte_text = list(text.encode())
    
    blocks = [pad_block(byte_text[i:i+16]) for i in range(0, len(byte_text), 16)]
    
    encrypted = []
    for block in blocks:
        encrypted.extend(aes_encrypt(block, round_keys))
    ...
    # decrypt and recover the original string
    decrypted = []
    for i in range(0, len(encrypted), 16):
        block = encrypted[i:i+16]
        decrypted.extend(aes_decrypt(block, round_keys))
    
    print("The decrypted string is")
    print(bytes(decrypted).decode(errors='ignore').rstrip('\x00'))
